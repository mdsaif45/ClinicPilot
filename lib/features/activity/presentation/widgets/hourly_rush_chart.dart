...
class _HourlyRushChartState extends State<HourlyRushChart>
  ...
  void _handleTouch(Offset localPosition, double plotWidth) {
    ...
    final index = (fraction * (totalSlots - 1)).round().clamp(
      0,
      totalSlots - 1,
    );
                          ...
                          builder: (context, _) {
                            return CustomPaint(
                              painter: _GoogleFitInteractiveChartPainter(
                                ...
                                dotColor: scheme.onSurfaceVariant.withValues(
                                  alpha: 0.4,
                                ),
...
class _GoogleFitInteractiveChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      Offset(0, topMargin + (plotHeight / 2)),
      Offset(plotWidth, topMargin + (plotHeight / 2)),
      gridPaint,
    );
    ...
    for (int i = 0; i < totalSlots; i++) {
      ...
      final centerX =
          totalSlots > 1 ? (i / (totalSlots - 1.0)) * plotWidth : 0.0;
    ...
    for (final (h, numStr, periodStr) in majorTicks) {
      ...
      canvas.drawLine(
        Offset(tickX, baselineY),
        Offset(tickX, baselineY + 3.5),
        axisPaint,
      );
      ...
      final textLeft = (tickX - (textPainter.width / 2)).clamp(
        0.0,
        plotWidth - textPainter.width,
      );
    ...
    if (selectedSlotIndex != null && selectedSlotIndex! < bins.length) {
      ...
      final cursorX =
          selectedCenterX ??
          ((selectedSlotIndex! / (totalSlots - 1.0)) * plotWidth);
