...
class InvestigationListView extends ConsumerStatefulWidget {
  ...
  const InvestigationListView({super.key, required this.patient, this.visitId});

  @override
  ConsumerState<InvestigationListView> createState() =>
      _InvestigationListViewState();
}
...
class _InvestigationListViewState extends ConsumerState<InvestigationListView> {
    ...
    showDialog(
      ...
      builder:
          (ctx) => AppConfirmDialog(
            ...
            message:
                'Are you sure you want to remove "${inv.testName}" recorded on ${Formatters.formatDate(inv.testDate ?? inv.createdAt)}?',
            confirmLabel: 'Delete',
            isDestructive: true,
            onConfirm: () async {
  ...
  Widget build(BuildContext context) {
      ...
      return Padding(
        ...
        child: AppCard(
          ...
          child: EmptyState(
            ...
            message:
                'Track pathology and biochemistry investigations with auto-flagging and trend charts.',
    ...
    final filteredList =
        _selectedFilterTest == null
            ? allInvestigations
            : allInvestigations
                .where((e) => e.testName == _selectedFilterTest)
                .toList();
                ...
                for (final name in testNames) ...[
                  FilterChip(
                    ...
                    onSelected: (selected) {
                      AppHaptics.selection();
                      setState(
                        () => _selectedFilterTest = selected ? name : null,
                      );
                    },
                  ),
                  const SizedBox(width: Spacing.xs),
...
class _ParameterTrendCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final numericReadings =
        investigations.where((i) => i.numericValue != null).toList()..sort(
          (a, b) =>
              (a.testDate ?? a.createdAt).compareTo(b.testDate ?? b.createdAt),
        );
                    ...
                    children: [
                      ...
                      Text(
                        Formatters.formatDate(
                          reading.testDate ?? reading.createdAt,
                        ),
                        style: theme.textTheme.labelSmall?.copyWith(
                          fontSize: 9,
              ...
              children:
                  attachments.map((path) {
                    ...
                    return ActionChip(
                      ...
                      label: Text(
                        'Report (${path.split(RegExp(r'[\\/]')).last})',
                      ),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: Spacing.xs),
                  Expanded(
                    child: Text(inv.notes!, style: theme.textTheme.bodySmall),
                  ),
                ],
              ),
            ),
