...
final diseaseAnalyticsProvider = Provider<AsyncValue<DiseaseAnalyticsSummary>>((ref) {
  ...
  return ref.watch(_rawDiseaseDataProvider).whenData((data) {
    final visits = data.visits.where((v) {
      final inRange = !v.visitDate.isBefore(range.start) && !v.visitDate.isAfter(range.end);
      final inClinic = activeClinic == null || v.clinicId == activeClinic.id;
      return inRange && inClinic;
    }).toList();

    final memos = data.memos.where((m) {
      final inRange = !m.memoDate.isBefore(range.start) && !m.memoDate.isAfter(range.end);
      final inClinic = activeClinic == null || m.clinicId == activeClinic.id;
      return inRange && inClinic;
    }).toList();
    ...
    final patientVisitCounts = <String, Map<String, int>>{}; // disease -> patientId -> count

    for (final v in visits) {
      final disease = (v.disease.trim().isEmpty ? 'General Consultation' : v.disease.trim());
      diseaseToPatients.putIfAbsent(disease, () => <String>{}).add(v.patientId);
      diseaseToVisits[disease] = (diseaseToVisits[disease] ?? 0) + 1;

      final pCounts = patientVisitCounts.putIfAbsent(disease, () => <String, int>{});
      pCounts[v.patientId] = (pCounts[v.patientId] ?? 0) + 1;
      if (pCounts[v.patientId]! > 1 || v.visitType.toLowerCase() == 'repeat') {
        diseaseToRepeatPatients.putIfAbsent(disease, () => <String>{}).add(v.patientId);
      }
    }
    ...
    for (final m in memos) {
      ...
      final disease = (primary != null && primary.trim().isNotEmpty)
          ? primary.trim()
          : 'General Consultation';
      diseaseToRevenue[disease] = (diseaseToRevenue[disease] ?? 0.0) + m.paidAmount;
    }

    // Combine all unique diseases
    final allDiseases = <String>{...diseaseToPatients.keys, ...diseaseToRevenue.keys};

    final statsList = <DiseaseStat>[];
    for (final d in allDiseases) {
      final pCount = diseaseToPatients[d]?.length ?? (patientMap.values.where((p) => p.primaryDisease == d).length);
    ...
    final totalRev = statsList.fold<double>(0.0, (sum, s) => sum + s.totalRevenue);
    final topRev = statsList.isNotEmpty ? statsList.first : null;

    final byVolume = [...statsList]..sort((a, b) => b.patientCount.compareTo(a.patientCount));
...
final _rawDiseaseDataProvider = StreamProvider<({
  ...
  final visitsStream = (db.select(db.visits)..where((t) => t.isDeleted.equals(false))).watch();

  return visitsStream.asyncMap((visits) async {
    final memos = await (db.select(db.cashMemos)..where((t) => t.isDeleted.equals(false))).get();
    final patients = await (db.select(db.patients)..where((t) => t.isDeleted.equals(false))).get();
