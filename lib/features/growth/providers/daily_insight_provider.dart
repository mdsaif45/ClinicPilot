...
final dailyInsightProvider = Provider<CoachInsight?>((ref) {
  ...
  if (overdueCount > 0) {
    generatedInsights.add(
      CoachInsight(
        id: 'recall_overdue',
        title:
            '$overdueCount ${overdueCount == 1 ? 'Patient' : 'Patients'} Overdue for Follow-up',
        message:
            'Send WhatsApp check-ins or follow-up reminders to ensure uninterrupted homeopathic treatment.',
  ...
  if (growth != null && growth.referralSourceCount.isNotEmpty) {
    final totalRefs = growth.referralSourceCount.values.fold<int>(
      0,
      (a, b) => a + b,
    );
    if (totalRefs >= 3) {
      final sortedRefs =
          growth.referralSourceCount.entries.toList()
      ...
      if (pct >= 35) {
        generatedInsights.add(
          CoachInsight(
            ...
            title:
                '${pct.toStringAsFixed(0)}% of New Patients from ${topRef.key}',
            message:
                '${topRef.key} is your strongest patient acquisition channel this period.',
  ...
  if (diseaseSummary != null &&
      diseaseSummary.stats.isNotEmpty &&
      diseaseSummary.totalRevenue > 0) {
    ...
    if (pct >= 25 && top.totalRevenue >= 1000) {
      generatedInsights.add(
        CoachInsight(
          ...
          title:
              '${top.disease} Generated ${pct.toStringAsFixed(0)}% of Revenue',
          message:
              'Yielded ${Formatters.formatCurrency(top.totalRevenue)} across ${top.patientCount} patients with a ${top.repeatRate.toStringAsFixed(0)}% repeat rate.',
  ...
  if (reviewStats != null && reviewStats.totalReviewed > 0) {
    generatedInsights.add(
      CoachInsight(
        id: 'review_milestone',
        title:
            '${reviewStats.totalReviewed} Google Reviews Logged (${reviewStats.averageRating.toStringAsFixed(1)} ★)',
        message:
            'Positive patient testimonials build online authority and attract high-trust organic leads.',
  ...
  if (campStats != null &&
      campStats.totalCamps > 0 &&
      campStats.aggregateRoi > 0) {
    generatedInsights.add(
      CoachInsight(
        id: 'camp_roi',
        title:
            'Health Camps Yielded +${campStats.aggregateRoi.toStringAsFixed(0)}% ROI',
        message:
            '${campStats.totalPatientsAcquired} patients acquired generated ${Formatters.formatCurrency(campStats.totalFollowUpRevenue)} in 90-day follow-up revenue.',
    ...
    if (margin >= 50) {
      generatedInsights.add(
        CoachInsight(
          ...
          message:
              'Practice earned ${Formatters.formatCurrency(profit.netProfit)} net after clinic rent and expenses.',
  ...
  if (generatedInsights.isEmpty) {
    generatedInsights.add(
      const CoachInsight(
        ...
        message:
            'Record every walk-in inquiry and send timely follow-up messages to build high-retention care loops.',
