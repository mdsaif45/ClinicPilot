  ...
  Future<void> _loadClinicGoals(String clinicId) async {
    ...
    final rev =
        await (db.select(db.settings)..where(
          (tbl) => tbl.key.equals('monthly_revenue_goal_$clinicId'),
        )).getSingleOrNull();
    if (rev != null && mounted) _revGoalController.text = rev.value;

    final pat =
        await (db.select(db.settings)..where(
          (tbl) => tbl.key.equals('monthly_new_patient_goal_$clinicId'),
        )).getSingleOrNull();
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Could not save clinic: $e')));
      }
      return;
    }
