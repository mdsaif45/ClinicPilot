...
class Complaints extends Table {
  ...
  DateTimeColumn get complaintDate =>
  ...
  TextColumn get side =>
      text().nullable()(); // Left, Right, Bilateral, Central, Not specified
  ...
  IntColumn get severity =>
      integer().withDefault(const Constant(5))(); // 1 to 10 scale
  TextColumn get status =>
      text().withDefault(
        const Constant('Active'),
      )(); // Active, Improving, Resolved, Recurrent
  TextColumn get beforeImages =>
      text().nullable()(); // JSON list of image paths
