  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (Migrator m, int from, int to) async {
      if (from < 2) {
        ...
        await customStatement(
          'UPDATE cash_memos SET is_deleted = 0 WHERE is_deleted IS NULL',
        );
        await customStatement(
          'UPDATE expenses SET is_deleted = 0, is_recurring = 0 WHERE is_deleted IS NULL',
        );
        await customStatement(
          'UPDATE expenses SET created_at = date WHERE created_at IS NULL',
        );
        ...
        await customStatement(
          'UPDATE clinics SET is_active = 1, is_deleted = 0 WHERE is_deleted IS NULL',
        );
        await customStatement(
          'UPDATE clinics SET created_at = cast(strftime("%s", "now") as integer) WHERE created_at IS NULL',
        );
