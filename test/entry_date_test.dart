...
void main() {
  ...
  Future<void> addMemo({
    ...
    return db.into(db.cashMemos).insert(CashMemosCompanion.insert(
          id: id,
  ...
  test('memo entered after midnight still reports in the month it was earned',
      () async {
    ...
    final march = await (db.select(db.cashMemos)
          ..where((t) =>
    ...
    final april = await (db.select(db.cashMemos)
          ..where((t) =>
  ...
  test('memos list newest first by the date on the memo, not the entry time',
      () async {
    ...
    final rows = await (db.select(db.cashMemos)
