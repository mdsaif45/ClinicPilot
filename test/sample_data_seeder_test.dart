...
void main() {
  ...
  test(
    'SampleDataSeeder populates 125 patients and rich practice history across 10 months',
    () async {
      ...
      final partialMemos =
          allMemos.where((m) => m.paidAmount < m.total).toList();
      expect(
        partialMemos.isNotEmpty,
        isTrue,
      ); // Some pending balances exist for realistic testing
      ...
      final rentExpenses =
          allExpenses.where((e) => e.category == 'Rent').toList();
      expect(
        rentExpenses.length,
        greaterThanOrEqualTo(20),
      ); // 10 months * 2 physical clinics
