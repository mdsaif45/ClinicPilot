...
void main() {
  ...
  group('BackupContainerService Unit & Round-Trip Tests', () {
    test('buildBackupBytes exports all 14 tables into valid .cpbak container', () async {
      ...
      await db.into(db.complaints).insert(
            Complaint(
              ...
              severity: 7,
              status: 'Active',
              beforeImages: jsonEncode(['/old/phone/storage/clinic_pilot/patient_media/patient-1/photo1.jpg']),
              isDeleted: false,
              createdAt: now,
              updatedAt: now,
            ),
      ...
      await db.into(db.investigations).insert(
            Investigation(
              ...
              unit: 'g/dL',
              flag: 'Normal',
              reportAttachments: jsonEncode(['/old/phone/storage/clinic_pilot/patient_media/patient-1/cbc_report.pdf']),
              testDate: now,
              isDeleted: false,
              createdAt: now,
              updatedAt: now,
      ...
      await db.into(db.referralContacts).insert(
            ReferralContact(
              id: 'ref-1',
              name: 'Dr. S. K. Sharma',
              category: 'Doctor',
      ...
      final restoreResult = await backupService.restoreFromBackupBytes(backupBytes);
      ...
      final restoredCaseRecords = await db.select(db.patientCaseRecords).get();
      expect(restoredCaseRecords.length, equals(1));
      expect(restoredCaseRecords.first.hpi, equals('Chronic migraine since 2 years'));

      final restoredComplaints = await db.select(db.complaints).get();
      expect(restoredComplaints.length, equals(1));
      expect(restoredComplaints.first.complaintName, equals('Migraine headache'));
    ...
    test('inspectBackup rejects invalid or non-ClinicPilot JSON files', () {
      final invalidJson = utf8.encode(jsonEncode({'app': 'OtherApp', 'data': []}));
      expect(
    ...
    test('restoreFromBackupBytes detects corrupted checksum in archive and aborts', () async {
      ...
      manifestMap['checksumSha256'] = 'tampered_invalid_sha256_checksum_value';

      final tamperedManifestBytes = utf8.encode(jsonEncode(manifestMap));
      archive.addFile(ArchiveFile('manifest.json', tamperedManifestBytes.length, tamperedManifestBytes));
