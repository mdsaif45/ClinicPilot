...
void main() {
  ...
  group('PeriodicBackupRunner Unit Tests', () {
    test('frequencyToDuration correctly maps all frequency options', () {
      expect(
        PeriodicBackupRunner.frequencyToDuration('Every 6 hours'),
        equals(const Duration(hours: 6)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Every day'),
        equals(const Duration(days: 1)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Every 2 days'),
        equals(const Duration(days: 2)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Once per week'),
        equals(const Duration(days: 7)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Twice per month'),
        equals(const Duration(days: 15)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Once per month'),
        equals(const Duration(days: 30)),
      );
      expect(
        PeriodicBackupRunner.frequencyToDuration('Unknown'),
        equals(const Duration(days: 7)),
      );
    });
    ...
    test(
      'executeBackup generates .cpbak file and rotates old backups',
      () async {
        ...
        await settingsBox.put('periodic_backup_enabled', true);
        await settingsBox.put(
          'periodic_backup_directory',
          customBackupDir.path,
        );
        ...
        final remainingBackups =
            await PeriodicBackupRunner.getExistingBackups();
