...
void main() {
  ...
  group('PracticeJournalProvider Unit Tests', () {
    test('groups entries by day with daily totals and descending sorting', () async {
      ...
      await db.into(db.cashMemos).insert(
            CashMemosCompanion.insert(
              ...
              memoDate: drift.Value(todayMidnight.add(const Duration(hours: 10, minutes: 15))),
              total: 1500,
              paymentMethod: 'upi',
            ),
          );
      ...
      // Yesterday Group
      final yesterdayGroup = groups.firstWhere((g) => g.dayLabel == 'Yesterday');
    ...
    test('filters journal by category and search query', () async {
      ...
      container.read(journalCategoryFilterProvider.notifier).state = JournalEventType.expense;
      var groups = container.read(practiceJournalProvider);
      expect(groups.isEmpty, isTrue);

      // Filter by Consultation (expect 1)
      container.read(journalCategoryFilterProvider.notifier).state = JournalEventType.consultation;
