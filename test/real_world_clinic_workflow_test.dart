...
void main() {
  ...
  group('Real-World Practice End-to-End Regression Tests', () {
    testWidgets(
      'Practice Setup -> Patient Registration with Single Clinic Auto-select',
      (tester) async {
        ...
        await tester.pumpWidget(
          UncontrolledProviderScope(
            ...
            child: MaterialApp(
              ...
              home: const Scaffold(body: AddPatientDialog()),
            ),
          ),
        );
    ...
    testWidgets(
      'Cash Memo: Full Payment vs Partial Payment with Pending Balance auto-calculation',
      (tester) async {
        tester.view.physicalSize = const Size(1200, 2400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        final patient =
            await (db.select(db.patients)
              ..where((t) => t.id.equals('patient_01'))).getSingle();
        ...
        expect(
          find.textContaining(
            'Note: Rs 200 will be recorded as Pending Due balance',
          ),
          findsOneWidget,
        );
    ...
    test(
      'Full Clinical Case Workflow: Master Record + Relational Complaints + Prescriptions + Labs + Referral CRM',
      () async {
        final container = ProviderContainer(
          overrides: [databaseProvider.overrideWithValue(db)],
        );
        ...
        final caseNotifier = container.read(
          caseRecordNotifierProvider.notifier,
        );
        final masterRecord = MasterCaseRecordData(
          ...
          hpi: const HpiDetails(
            progression:
                'Pain began 2 years ago, gradually worsening. Known Type 2 Diabetes.',
          ),
          pastHistory: const PastHistoryDetails(
            surgeries: 'Rectal abscess surgically drained 5 years ago.',
          ),
          familyHistory: const FamilyHistoryDetails(
            father: 'Father had Diabetes and Hypertension.',
          ),
          ...
          miasmaticAnalysis: const MiasmaticAnalysis(
            ...
            sycoticFeatures:
                'Warts on forehead and tongue, joint stiffness < cold air',
            dominantMiasm: 'Sycotic',
          ),
          caseTotality: const CaseTotality(
            characteristicSymptoms:
                'Right-sided joint stiffness, warts, hot patient, profuse sweat',
            rubricsSelected:
                'Extremities; pain; hip; right • Generals; thermal; hot',
        ...
        final savedCase = await container.read(
          patientCaseRecordProvider('p_saif').future,
        );
        ...
        final complaintNotifier = container.read(
          complaintNotifierProvider.notifier,
        );
        ...
        final rxNotifier = container.read(
        ...
        final invNotifier = container.read(
          investigationNotifierProvider.notifier,
        );
        ...
        final crmNotifier = container.read(
          referralCrmNotifierProvider.notifier,
        );
