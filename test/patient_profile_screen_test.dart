...
void main() {
  ...
  testWidgets(
    'PatientProfileScreen renders header, metrics, and segmented tabs',
    (tester) async {
      ...
      expect(
        find.text('Pending ${Formatters.formatCurrency(100)}'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'PatientProfileScreen renders populated clinical complaints, Rx, lab tests, and case sheet tabs without errors',
    (tester) async {
      ...
      final testComplaint2 = Complaint(
        ...
        complaintIndex: 2,
        complaintDate:
            null, // Legacy row with null date to verify backward compatibility
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            patientComplaintsProvider('p-1').overrideWith(
              (ref) => Stream.value([testComplaint1, testComplaint2]),
            ),
  ...
  testWidgets(
    'PatientProfileScreen dynamically updates FAB per tab and opens ScheduleFollowUpDialog on Follow-ups tab',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
