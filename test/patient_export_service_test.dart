...
void main() {
  ...
  group('PatientExportService Unit & Integration Tests', () {
    test(
      'fetchPatientExportRows aggregates full clinical and financial data per patient',
      () async {
        ...
        final exportRows = await PatientExportService.fetchPatientExportRows(
          db,
        );
        ...
        expect(
          row.activeComplaints,
          contains('Skin Scaling & Itching (Severity 8/10)'),
        );
    ...
    test(
      'buildMultiSheetPatientXlsx produces valid workbook with 9 populated sheets and clinical summaries',
      () async {
        ...
        final caseRecord = MasterCaseRecordData(
          ...
          hpi: const HpiDetails(
            chronologicalDevelopment:
                'Started 2 years ago after high job stress.',
          ),
          pastHistory: const PastHistoryDetails(
            majorIllnesses: 'Typhoid (2020)',
            surgeries: 'None',
          ),
        ...
        final xlsxBytes = await PatientExportService.buildMultiSheetPatientXlsx(
          db,
        );
        ...
        expect(
          decoded.tables.containsKey('Physical & Mental Generals'),
          isTrue,
        );
