...
void main() {
  ...
  group('Online & Remote Teleconsultation Tests', () {
    test('registerPatient with online mode creates clinic_online and assigns ONL serial', () async {
      ...
      final onlineClinic = await (db.select(db.clinics)..where((c) => c.id.equals('clinic_online'))).getSingle();
      expect(onlineClinic.name, contains('Online'));
      expect(onlineClinic.monthlyRent, equals(0.0));

      // Verify visit is marked online
      final visits = await (db.select(db.visits)..where((v) => v.patientId.equals(patient.id))).get();
    ...
    test('registerPatient works seamlessly when patient has no phone number (Google Meet / Privacy)', () async {
      final container = ProviderContainer(
        overrides: [databaseProvider.overrideWithValue(db)],
      );
      addTearDown(container.dispose);
    ...
    testWidgets('AddPatientDialog renders consultation mode toggle and toggles online fields', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            masterDiseasesListProvider.overrideWith((ref) => kDefaultHomeopathicDiseases),
          ],
          child: MaterialApp(
      ...
      expect(find.text('In-Clinic Visit', skipOffstage: false), findsOneWidget);
      expect(find.text('Online / Remote', skipOffstage: false), findsOneWidget);
      ...
      expect(find.text('Locality / Area', skipOffstage: false), findsOneWidget);
      ...
      expect(find.text('City / State / Location', skipOffstage: false), findsOneWidget);
      expect(find.text('Consultation Medium', skipOffstage: false), findsOneWidget);

      await tester.tap(find.text('Cancel'));
      await tester.pumpAndSettle();
    });

    testWidgets('AddPatientDialog auto-selects Online / Remote when active clinic is clinic_online', (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            databaseProvider.overrideWithValue(db),
            clinicsStreamProvider.overrideWith((ref) => Stream.value([clinic, onlineClinic])),
            masterDiseasesListProvider.overrideWith((ref) => kDefaultHomeopathicDiseases),
          ],
          child: MaterialApp(
            theme: AppTheme.lightTheme,
            home: Builder(
              builder: (ctx) => Scaffold(
                body: ElevatedButton(
                  onPressed: () => showDialog(
                    context: ctx,
                    builder: (_) => const AddPatientDialog(initialClinicId: 'clinic_online'),
                  ),
    ...
    testWidgets('NewCashMemoDialog pre-fills UPI and online clinic for online patient', (tester) async {
      tester.view.physicalSize = const Size(1200, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            databaseProvider.overrideWithValue(db),
            clinicsStreamProvider.overrideWith((ref) => Stream.value([onlineClinic])),
            patientsStreamProvider.overrideWith((ref) => Stream.value([onlinePatient])),
          ],
          child: MaterialApp(
            theme: AppTheme.lightTheme,
            home: Builder(
              builder: (ctx) => Scaffold(
                body: ElevatedButton(
                  onPressed: () => showDialog(
                    context: ctx,
                    builder: (_) => NewCashMemoDialog(initialPatient: onlinePatient),
                  ),
